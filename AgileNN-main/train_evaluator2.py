...
def construct_effnetv2(image_size=224, num_classes=100):
    effnetv2 = tf.keras.applications.efficientnet.EfficientNetB0(
        include_top=False,
        weights='imagenet',
        input_shape=(image_size, image_size, 3),
         ) # expected pixel value range [-1, 1]
...
def train_effnetv2_on_imagenet200(run_name, logdir):
    ...
    ds = tfds.load('tiny_imagenet_dataset',  as_supervised=True,data_dir='../dataset/',download=False)
    ...
    ds_test = ds['validation'].map(prep, num_parallel_calls=tf.data.AUTOTUNE)\
                            .batch(8*4)\
                            .prefetch(buffer_size=tf.data.AUTOTUNE)
